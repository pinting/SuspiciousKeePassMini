...
@objc public class ALOptions: NSObject { // The structure used to display the controller
...
public class AppLocker: UIViewController {
    ...
    private var validateFailedCount = 3
    ...
    private func drawing(isNeedClear: Bool, tag: Int? = nil) { // Fill or cancel fill for indicators
        ...
        UIView.animate(withDuration: ALConstants.duration, animations: {
            pinView?.backgroundColor = isNeedClear ? .clear : col
            //pinView?.tintColor = col
        }) { _ in
    ...
    private func validateModeAction() {
        if pin == savedPin {
            ...
            validateFailedCount -= 1
            submessageLabel.text = "\(validateFailedCount) more attempts"
            onFailedAttempt?(mode)
            incorrectPinAnimation()
        }
    }
