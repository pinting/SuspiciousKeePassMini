    ...
    private func _layoutSubViews() {
        ...
        if #available(iOS 13.0, *) {
            if (appSettings.darkEnabled()) {
                self.appearance.backgroundType = .blurExtraLight
                self.appearance.messageTextColor = .black
                self.appearance.timeTextColor = .black
                self.appearance.titleTextColor = .black
            }else{
                self.appearance.backgroundType = .blurDark
