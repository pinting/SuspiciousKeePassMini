...
@objc public class FaviconFinder: NSObject {
    ...
    @objc public init(url: URL, logEnabled: Bool = false, image: UIImage ,btn: UIButton) {
        self.url = url
       self.preferredType = FaviconDownloadType.html
        self.preferences = [
    ...
    @objc func downloadico()
    {
            self.downloadFavicon { result in
                    switch result {
                    case .success(let favicon):
                        print("URL of Favicon: \(favicon.url)")
                        DispatchQueue.main.async {
                            self.image = favicon.image
                            self.btn.setImage(favicon.image, for: .normal)
                        }
